...
def load(args):
    ...
    for dirname in args.directory:
        ...
        # Invert metric for evaluation
        frame["FOM"] = -1 * frame["FOM"]
...
def visualizations(frames, args):
    ...
    for frame, color, name, idx in zip(frames, matplotlib.colors.TABLEAU_COLORS, names, np.arange(len(frames))):
        if args.monotonic:
            frame = frame.sort_values(by=['FOM',]).reset_index(drop=True)
            frame['index'] = frame.index
        fline = sns.lineplot(frame, x='index', y='FOM', estimator=None, marker='+', label=name,
                             color=color, markeredgecolor=color, linestyle='--', linewidth=1)
        if args.flops_only:
            ax.hlines(frame['FOM'].quantile(args.quantile[idx]), xmin=0, xmax=max_frame_index, color=color)
