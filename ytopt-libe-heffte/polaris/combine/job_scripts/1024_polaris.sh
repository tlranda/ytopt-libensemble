...
for n_workers in ${workers[@]}; do
    ...
    call="python libEwrapper.py --mpi-ranks ${n_ranks} --worker-timeout 300 --application-scale ${app_scale} --gpu-enabled --ensemble-workers ${n_workers} --max-evals 200 --configure-environment craympi --machine-identifier polaris-gpu --ensemble-dir-path Polaris_${n_ranks}r_${app_scale}a --ensemble-path-randomization --libensemble-export libE_Run_.py --libensemble-randomization --launch-job --display-results";
    date;
    echo "${call}";
    eval "${call}";
    date;
