...
for n_workers in ${workers[@]}; do
    ...
    call="python libEwrapper.py --mpi-ranks ${n_ranks} --worker-timeout 300 --application-scale ${app_scale} --ensemble-workers ${n_workers} --max-evals 200 --configure-environment craympi --machine-identifier cpu-polaris --system polaris --ens-dir-path CPU_Polaris_${n_ranks}r_${app_scale}a --launch-job --display-results";
    date;
    echo "${call}";
    eval "${call}";
    date;
