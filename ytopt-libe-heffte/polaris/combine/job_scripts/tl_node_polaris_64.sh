...
for n_workers in ${workers[@]}; do
    ...
    call="python libEwrapper.py --mpi-ranks ${n_ranks} --worker-timeout 300 --application-scale ${app_scale} --gpu-enabled --ensemble-workers ${n_workers} --max-evals 30 --configure-environment craympi --machine-identifier polaris-gpu --system polaris --ens-dir-path Polaris_${n_ranks}r_${app_scale}a_TL --ens-template run_gctla.py --ens-script qsub_tl.batch --gc-input logs/PolarisSourceTasks/*n_${str_app}a/manager_results.csv --gc-ignore logs/PolarisSourceTasks/Polaris_${str_nodes}n_${str_app}a/manager_results.csv --gc-initial-quantile 0.8";
    date;
    echo "${call}";
    eval "${call}";
    date;
