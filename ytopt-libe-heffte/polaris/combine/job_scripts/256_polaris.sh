...
for n_workers in ${workers[@]}; do
    ...
    call="python libEwrapper.py --mpi-ranks ${n_ranks} --worker-timeout 300 --application-scale ${app_scale} --gpu-enabled --ensemble-workers ${n_workers} --max-evals 100 --configure-environment craympi --machine-identifier polaris-gpu --ensemble-dir-path PolarisTimeout_${app_scale} --ensemble-path-randomization --launch-job --display-results";
    date;
    echo "${call}";
    eval "${call}";
    date;
