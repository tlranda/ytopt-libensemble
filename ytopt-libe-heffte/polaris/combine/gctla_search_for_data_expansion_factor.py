...
MACHINE_IDENTIFIER = "thetalogin4"
...
#topology_len = len(topology_cache[constraints[1]['constraint_parameters']['high_value']])
topology_len = len(topology_cache[user_args['constraint-sys']])
# Square topology len as there are two parameters based on the topologies
possible_configurations = uncasted_space_size * len(sequence) * (topology_len ** 2)
...
# DEBUG: Try to artificially expand the dataset and see if that leads to better probability
n_per_file = 50
n_to_add = n_per_file * len(data_files)
print(f"Attempting to add {n_per_file} records per file, for a total of {n_to_add} additional records")
assert len(data_trimmed) + n_to_add <= possible_configurations # Can't add more than actually exist
warnings.simplefilter('ignore')
expanded_metadata = SingleTableMetadata()
expanded_metadata.detect_from_dataframe(data_trimmed)
expanded_model = GaussianCopula(expanded_metadata, enforce_min_max_values=False)
expanded_model.add_constraints(constraints=constraints)
expanded_model.fit(data_trimmed)
warnings.simplefilter('default')
new_data = expanded_model.sample(num_rows=n_to_add)
og_len = len(data_trimmed)
data_trimmed = pd.concat((data_trimmed, new_data))
print(f"Dataset length {og_len} --> {len(data_trimmed)}")
...
while True:
    ...
    if subideal > subpop:
        print(f"Autotuning budget indeterminate at quantile {data_quantile} (subideal {subideal} > subpop {subpop})")
        suggested_budget = user_args['max-evals']
    else:
        suggested_budget = 0
