...
import numpy as np, pandas as pd, uuid, time, copy, subprocess
...
def csvs_to_gptune(fnames, tuning_metadata):
    ...
    for fname in fnames:
        ...
        # TEMP NEW ADDITION: Reconvert to old format
        csv.insert(0,'p1',csv['p1x'].values)
        csv.drop(columns=[f'p1{A}' for A in 'xyz'])
        for col in [f'p{N}' for N in '789']:
            csv[col] = csv[col+'_float'].values
