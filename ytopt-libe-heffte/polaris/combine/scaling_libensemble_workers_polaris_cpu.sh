...
mpi_ranks=( 64 128 256 512 1024 4096 );
#  nodes =   1   2   4   8   16   64
workers=( 4 );
...
for n_workers in ${workers[@]}; do
    ...
    call="python libEwrapper.py --mpi-ranks ${n_ranks} --worker-timeout 300 --application-scale ${app_scale} --cpu-override 64 --ensemble-workers ${n_workers} --max-evals 100 --configure-environment craympi --machine-identifier polaris-cpu --ensemble-dir-path Polaris_${mpi_ranks}r_${app_scale}a --ensemble-path-randomization --launch-job --display-results";
    date;
    echo "${call}";
    #eval "${call}";
    date;
