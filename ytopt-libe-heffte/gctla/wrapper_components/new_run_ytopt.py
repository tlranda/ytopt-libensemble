...
def parse_custom_user_args_in(user_args_in):
    ...
    # Pathing update for node-list-file
...
def prepare_libE(nworkers, libE_specs, user_args_in):
    ...
    hostfile = None if 'node-list-file' not in user_args.keys() else user_args['node-list-file']
    ...
    if hostfile is not None:
        symlinkable.append(pathlib.Path(hostfile))
    ...
    if hostfile is not None:
        MACHINE_INFO['nodelist'] = hostfile
