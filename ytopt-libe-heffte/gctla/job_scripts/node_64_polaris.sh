...
#PBS -l select=129:system=polaris
...
# nodes =    64
workers=( 4 );
# MAX nodes = (32 * 4 + 1) == (128 + 1) = 129 (<3% cluster capacity)
...
for n_workers in ${workers[@]}; do
    ...
    call="python libEwrapper.py --mpi-ranks ${n_ranks} --worker-timeout 300 --application-scale ${app_scale} --gpu-enabled --ensemble-workers ${n_workers} --max-evals 200 --configure-environment craympi --machine-identifier polaris-gpu --system polaris --ens-dir-path Polaris_${n_ranks}r_${app_scale}a --launch-job --display-results";
    date;
    echo "${call}";
    eval "${call}";
    date;
