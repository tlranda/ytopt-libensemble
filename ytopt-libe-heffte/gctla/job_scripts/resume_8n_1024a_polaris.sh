...
#PBS -l select=33:system=polaris
...
workers=( 4 );
# MAX nodes = 4 * 8 + 1 = 33 (6% cluster capacity)
...
for n_workers in ${workers[@]}; do
    ...
    call="python libEwrapper.py --mpi-ranks ${n_ranks} --worker-timeout 300 --application-scale ${app_scale} --gpu-enabled --ensemble-workers ${n_workers} --max-evals 200 --configure-environment craympi --machine-identifier polaris-gpu --system polaris --ens-dir-path Polaris_Resume_${n_ranks}r_${app_scale}a --launch-job --display-results --resume logs/PolarisSourceTasks/Polaris_${str_nodes}n_${str_app}a/manager_results.csv";
    date;
    echo "${call}";
    eval "${call}";
    date;
